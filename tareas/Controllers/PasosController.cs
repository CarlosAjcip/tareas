...
namespace tareas.Controllers
{
    ...
    public class PasosController : ControllerBase
    {
        ...
        {
            var usuarioId = serviciosUsuarios.ObtenerUsuarioId();
            var paso = await context.Pasos.Include(p => p.Tarea).FirstOrDefaultAsync(x => x.Id == id);
        ...
        {
            var usuarioId = serviciosUsuarios.ObtenerUsuarioId();
            var tarea = await context.Tareas.FirstOrDefaultAsync(x => x.Id == tareaId && x.UsuarioCreacionId == usuarioId);
            ...
            var idsPasosNoPertenecenalTare = ids.Except(pasosIds).ToList();

            if (idsPasosNoPertenecenalTare.Any())
            {
