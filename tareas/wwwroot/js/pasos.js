...
async function actualizarPaso(data, id) {
    const respueata = await fetch(`${urlPasos}/${id}`, {
        body: data,
        method: "PUT",
        headers: {
            'Content-Type': 'application/json'
...
function manejarClickCheckboxPaso(paso) {
    ...
    const tarea = obtenerTareaEnEdicion();
    let pasosReealizadosActual = tarea.pasosRealizados();

    if (paso.realizado()) {
        pasosReealizadosActual++;
    } else {
        pasosReealizadosActual--;
    }

    tarea.pasosRealizados(pasosReealizadosActual);
    return true;
}
